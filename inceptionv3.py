    ...
    def batch_ops(X, y):
        ...
        with tf.name_scope('transformation'):
            ...
            if rotation_range > 0:
                ...
                transforms += [tfa.image.transform_ops.angles_to_projective_transforms(angles, intermediate_size[0],
                                                                                       intermediate_size[1])]
...
def flow_from_dataframe(idg,
    ...
    while True:
        ...
        for i in range(max(in_len // 32, 1)):
            ...
            #     sess.run(next_batch)
            yield tf.compat.v1.data.make_one_shot_iterator(idg(files_ds).repeat()).get_next()
...
for (idx, c_ax) in enumerate(m_axs.flatten()):
    ...
    c_ax.set_title('Actual Severity: {}\n{}'.format(test_Y_cat[idx],
                                                    '\n'.join(['Predicted %02d (%04.1f%%): %s' % (
                                                    k, 100 * v, '*' * int(10 * v)) for k, v in
                                                               sorted(enumerate(pred_Y[idx]),
                                                                      key=lambda x: -1 * x[1])])), loc='left')
    c_ax.axis('off')
